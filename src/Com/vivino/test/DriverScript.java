...
import Com.vivino.test.Xls_Reader;
import Com.vivino.test.DriverScript;
import Com.vivino.test.Keywords;
...
public class DriverScript {
	...
	public static void main(String[] args) throws IOException, NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		 FileInputStream fs = new FileInputStream("/Users/rishabhbakshi/Desktop/Hybrid/Vivino_web/config.properties");
	        CONFIG= new Properties();
	        CONFIG.load(fs);

	        fs = new FileInputStream("/Users/rishabhbakshi/Desktop/Hybrid/Vivino_web/or.properties");
	        OR= new Properties();
	        OR.load(fs);
	...
	public void start()throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException{
        ...
        suiteXLS= new Xls_Reader("/Users/rishabhbakshi/Desktop/Hybrid/Vivino_web/Suite.xlsx");
        ...
        for(currentSuiteID=2;currentSuiteID<=suiteXLS.getRowCount(Constants.TEST_SUITE_SHEET);currentSuiteID++)
        {
            ...
            if(suiteXLS.getCellData(Constants.TEST_SUITE_SHEET, Constants.RUNMODE, currentSuiteID).equals(Constants.RUNMODE_YES))
            {
            	//  APP_LOGS.debug("******Executing the Suite******"+suiteXLS.getCellData(Constants.TEST_SUITE_SHEET, Constants.Test_Suite_ID, currentSuiteID)); // Sheet name , Col name, Row no.
            	  currentTestSuiteXLS =new Xls_Reader("/Users/rishabhbakshi/Desktop/Hybrid/Vivino_web/Login Page.xlsx");//Read the xls file which is mention in TSID in Suit.xls file Runmode=Y
            	  for(currentTestCaseID=2;currentTestCaseID<currentTestSuiteXLS.getRowCount("Test Cases");currentTestCaseID++)
            	  { //Its a loop for read data form Test cases sheet row wise
